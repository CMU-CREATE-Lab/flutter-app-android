...
public class AppLandingActivity extends BaseNavigationActivity implements FlutterConnectListener
{
	...
	public SerialBleHandler serialBleHandler;
	...
	public GenericBleScanner.ScannerCallback scannerCallback = new GenericBleScanner.ScannerCallback()
	{
		...
		{
			if (bluetoothDevice.getName() == null || !bluetoothDevice.getName().contains("FL6"))
				return;
			...
			if (!bleDevices.contains(bluetoothDevice))
			{
				...
				String name = NamingHandler.generateName(getApplicationContext(), bluetoothDevice.getAddress());
	...
	private void bleScan(boolean enabled)
	{
		if (enabled)
		{
			bleDevices.clear();
		}
		genericBleScanner.scanLeDevice(enabled, scannerCallback);
	}
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		//initialize generic ble connection handler and device scanner
		serialBleHandler = new SerialBleHandler(this);
		genericBleScanner = serialBleHandler.getScanner();

		//request for bluetooth permission to be enabled
	...
	{
		Log.d(LOG_TAG, "onClickScan");
		bleScan(true); //for generic ble devices
		scanForDevice(true); //for melody smart ble devices
		if (noFlutterFoundTimer != null)
		{
			noFlutterFoundTimer.cancel();
		}
	...
	{
		Log.d(LOG_TAG, "AppLandingActivity.onFlutterConnected");
	...
	{
		Log.d(LOG_TAG, "AppLandingActivity.onFlutterDisconnected");
