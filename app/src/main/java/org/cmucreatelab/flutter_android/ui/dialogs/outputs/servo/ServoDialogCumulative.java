 ...
 *
 * ServoDialogStateHelper implementation with Cumulative relationship.
 *
 */
public class ServoDialogCumulative extends ServoDialogStateHelper {


    ServoDialogCumulative(Servo servo) {
        super(servo);
    }


    public static ServoDialogStateHelper newInstance(Servo servo) {
        return new ServoDialogCumulative(servo);
    }


    @Override
    public void updateView(ServoDialog dialog) {
        Log.v(Constants.LOG_TAG,"ServoDialogCumulative.updateView");
        SettingsCumulative settings = (SettingsCumulative) getServo().getSettings();
        Sensor sensor = settings.getSensor();

        if (getServo().getSettings().getRelationship().getClass() != Cumulative.class) {
    ...
    public void clickMin(ServoDialog servoDialog) {
        DialogFragment dialog = MinPositionDialog.newInstance(Integer.valueOf(((SettingsCumulative)getServo().getSettings()).getOutputMin()), servoDialog);
    ...
    public void clickMax(ServoDialog servoDialog) {
        DialogFragment dialog = MaxPositionDialog.newInstance(Integer.valueOf(((SettingsCumulative)getServo().getSettings()).getOutputMax()), servoDialog);
        dialog.show(servoDialog.getFragmentManager(), "tag");
    }
    ...
    public void setAdvancedSettings(AdvancedSettings advancedSettings) {
        ((SettingsCumulative)getServo().getSettings()).setAdvancedSettings(advancedSettings);
    }
    ...
    public void setMaximumPosition(int maximumPosition, TextView description, TextView item) {
        Log.w(Constants.LOG_TAG,"ServoDialogCumulative.setMaximumPosition: attribute not implemented");
        SettingsCumulative settings = (SettingsCumulative) getServo().getSettings();
