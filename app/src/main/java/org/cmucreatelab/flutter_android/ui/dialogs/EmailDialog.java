    ...
    public void onClickSendEmail() {
        ...
        if (Pattern.matches(
            ...
            if (Constants.SEND_EMAIL_AS == Constants.MailerType.INTENT) {
                ...
                String flutterName = "Unknown";
                Session session = GlobalHandler.getInstance(getActivity().getApplicationContext()).sessionHandler.getSession();
                if (session != null) {
                    flutterName = session.getFlutter().getName();
                }

                EmailHandler.sendEmailServer(getActivity(), email.getText().toString(), message.getText().toString(), FileHandler.getFileFromDataSet(globalHandler, currentDataLog), flutterName);
            } else {
